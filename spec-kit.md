...
src/
  ...
  _includes/
    ...
    layouts/
      ...
      blog.vto        # Blog post shell — OG article meta, JSON-LD BlogPosting, reading-time, ToC
      tag.vto         # Tag index shell — lists all posts for a given tag (generated by tags.page.ts)
  blog/               # Markdown blog posts (001-…md … 017-…md) + tags.page.ts generator
  *.vto               # Page templates: index, blog, projects, experience, certifications, uses, 404
  resume.pdf          # Compiled resume PDF — tracked in git, served at /resume.pdf
  styles/
    main.css          # Single CSS file; processed by LightningCSS
  assets/images/      # Static images
  _site/              # Build output (gitignored; Vercel builds from source)
_cache/               # Lume build cache (gitignored)
.opencode/
  opencode.json       # Project-level OpenCode config: MCP servers (context7, gh_grep)
  agents/
    blog-post.md      # Subagent: create/fix blog posts — enforces heading rules & frontmatter
    content.md        # Subagent: update _data/*.json, uses.vto, index.vto
    resume.md         # Subagent: edit resume.json, compile PDF, stage src/resume.pdf
  commands/
    add-blog.md       # /add-blog <title> — delegates to @blog-post
    update-resume.md  # /update-resume — delegates to @resume
    deploy.md         # /deploy <message> — git add -A && commit && push
docs/
  ADDING_BLOG.md      # How to add a new blog post
# Local-only (gitignored — never commit these):
resume.json           # Resume data source for Typst
main.typ              # Typst template (imports kiresume:0.1.17)
```
...
| Task | Command | Notes |
|---|---|---|
| `dev` | `deno task dev` | Alias for `serve` — builds SW then starts Lume dev server with live reload |
| `serve` | `deno task serve` | Builds SW then starts Lume dev server |
| `build` | `deno task build` | `deno audit` → production Lume build (minifyHTML enabled) into `_site/` |
| `build:sw` | `deno task build:sw` | Generates SW only via `scripts/build-sw.js` |
...
| `resume` | `deno task resume` | `$HOME/.local/bin/typst compile main.typ src/resume.pdf` |
...
- **Preprocess step** (`.md` pages): (1) lowercases tags; (2) computes `readingTime` (word count ÷ 200, min 1) from stripped Markdown source and stores it on `page.data.readingTime`.
- **`src/blog/tags.page.ts`**: generator that produces one page per unique tag at `/blog/tags/[tag]/` using `layouts/tag.vto`.
...
- `blog.vto`: JSON-LD `BlogPosting` schema with `datePublished`, `author`, `publisher`, `image` (frontmatter `cover` URL) and `keywords`.
...
- ~~**Reading-time estimate**: compute word-count in the `_config.ts` preprocess step, expose a `readingTime` data field, render in `blog.vto` header.~~ **Done** — stripped-Markdown word count at 200 wpm, rendered server-side; JS fallback retained for word-count display.
- ~~**Tag index pages**: use Lume's `paginate` helper to generate `/blog/tags/[tag]/` listing pages.~~ **Done** — `src/blog/tags.page.ts` generator + `layouts/tag.vto` layout; card tags now link to these pages. Note: file must use `.page.ts` extension (not plain `.ts`) for Lume to treat it as a page generator.
- ~~**OG image generation**: auto-generate per-post OG images (Satori / canvas) instead of relying on cover images.~~ **Future idea** — deferred: `og_images` plugin generates PNGs into `_site/` at build time; since `_site/` is committed to the repo, each rebuild would bloat git history with binary assets.
- ~~**Dark-mode default**: supplement the `localStorage` toggle with `prefers-color-scheme` as the initial default.~~ **Done** — falls back to OS preference when no explicit choice is saved; live `matchMedia` listener added.
- ~~**CI lint gate**: add `deno task lint && deno task check` as a required step in `deploy.yml` before the build.~~ **Done** — prefixed in the `deploy` task in `deno.json`.
- ~~**SW cache version**: tie `CACHE_VERSION` in `build-sw.js` to the build timestamp or git SHA for automatic cache-busting on each deploy.~~ **Done** — `scripts/build-sw.js` reads `git rev-parse --short=8 HEAD`; falls back to compact date string.
- ~~**`/uses` page**: a dedicated page listing tools, hardware, and software in use.~~ **Done** — `src/uses.vto` with 7 sections; linked from homepage nav.
- ~~**Blog Table of Contents**: auto-generate ToC from `##`/`###` headings in blog posts.~~ **Done** — `blog.vto` generates ToC via JS if post has ≥3 headings; highlights active section on scroll.
- ~~**Tech stack row**: visual tag strip on homepage showing core technologies.~~ **Done** — 14 tags rendered in `src/index.vto` with accent-tinted pill styles.
...
| `_site/` gitignored | Vercel builds from source on its own infrastructure; `_site/` is never committed |
| `_cache/` gitignored | Lume build cache is local-only; should never be tracked in version control |
| `src/resume.pdf` tracked in git | Vercel serves it as a static asset; the PDF must exist in the repo. Source files (`resume.json`, `main.typ`) are gitignored and local-only — regenerate with `deno task resume` |
| OpenCode agents + commands in `.opencode/` | Project-level agents (`blog-post`, `content`, `resume`) and slash commands (`/add-blog`, `/update-resume`, `/deploy`) for consistent, rule-enforced AI-assisted development |
| Context7 + gh_grep MCPs | Zero-auth remote MCPs: Context7 provides live Deno/TS docs; gh_grep searches real GitHub code examples — both reduce hallucinated API usage |

---

_Last updated: 2026-03-05 — Added /uses page, blog ToC, tech stack row, resume pipeline, OpenCode agents/commands/MCPs, _cache gitignored_
